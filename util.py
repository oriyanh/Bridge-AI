...
class PriorityQueueWithFunction(PriorityQueue):
  ...
  """
    ...
    def push(self, item, priority=0):
        """Adds an item to the queue with priority from the priority
        function"""
...
class Counter(dict):
  ...
  """
    ...
    def __sub__(self, y):
        """ Subtracting a counter from another gives a counter with the
        union of all keys and counts of the second subtracted from counts of
        the first.
        """
        addend = Counter()
        for key in self:
  ...
  """
    ...
    if dots > 0:
        module_name, obj_name = \
            '.'.join(name.split('.')[:-1]), name.split('.')[-1]
